...
function O(T) {
  for (let [Y, r] of Object.entries(C)) T = T.replaceAll(r, Y);
  return T;
}
function P(T) {
  let Y = T.db, r = T.toplevel, s = (n, a) => {
    if (n === "") return a;
    let e = n.match(/^(\d+)\/(.+)$/);
    if (e) {
  ...
  for (let [n, a] of Object.entries(Y)) if (typeof a == "object" && !Array.isArray(a)) {
    i[`${n}`] = {};
    let e = a;
...
function p(T) {
  let Y = P(Z), r = T.split("/"), s;
  try {
    if (r.length === 3) {
      let n = Y[`${r[0]}`];
      if (n && !Array.isArray(n)) {
        let a = n[`${r[1]}`];
        a && !Array.isArray(a) && (s = a[`${r[2]}`]);
      }
    } else if (r.length === 2) {
      let n = Y[`${r[0]}`];
      n && !Array.isArray(n) && (s = n[`${r[1]}`]);
    } else s = Y[`${r[0]}`];
    if (s == null || !Array.isArray(s) || s.length < 2) throw new Error("Given time zone not valid.");
    let i = s;
...
var h = z;
function j(T, Y = false) {
  let r = p(T);
  if (typeof r == "string" || !r[1] || r[1] === "") return "";
  let s = r, i = (function() {
    return s[0] == "" ? T : s[0];
  })(), n = "TZID=" + i, a = [`BEGIN:VTIMEZONE\r
TZID:` + i + `\r
X-LIC-LOCATION:` + i + `\r
LAST-MODIFIED:` + s[1].replace(/[^\w\-:,;=+/<>]/g, "").replace(/<br>/g, `\r
`) + "END:VTIMEZONE", n];
  return Y ? JSON.stringify(a) : a;
}
function I(T, Y, r) {
  let s = p(T);
  if (typeof s == "string" || s[1] == null || s[1] == "") return "";
  let i = s;
  ...
  if (!i[1].match(/BEGIN:DAYLIGHT/i)) {
    let u = i[1].match(/TZOFFSETTO:([+|-]\d{4})/i);
    return u ? u[1] : "";
  }
  let n = Y + "T" + r + ":00", a = new Date(n), e = a.getFullYear(), t = a.getMonth() + 1, c = a.getDate(), A = a.getHours(), b = i[1].replace(/[^\w\-:,;=+/<>]/g, "").split("<br>"), o = { 1: {}, 2: {} }, l = 0;
  for (let u = 0; u < b.length; u++) {
    let f = b[u];
    if (f.startsWith("TZOFFSETTO") && (l++, o[`${l}`] && (o[`${l}`].offset = f.split(":")[1])), f.startsWith("DTSTART") && o[`${l}`] && (o[`${l}`].hour = parseInt(f.substr(17, 2))), f.startsWith("RRULE")) {
      let m = f.split(";"), k = m[1].split("=")[1], L = parseInt(k);
      o[`${l}`] && (o[`${l}`].month = L, o[`${l}`].day = m[2].split("=")[1]);
    }
  }
  ...
  let B = (function() {
    return Object.keys(o).find((u) => o[`${u}`].month == t);
  })();
  if (!B) return E.offset;
  let M = o[`${B}`], y = ["SU", "MO", "TU", "WE", "TH", "FR", "SA"], _ = new Date(e, t, 0).getDate(), S = new Date(e, t - 1, 1).getDay(), g = { SU: {}, MO: {}, TU: {}, WE: {}, TH: {}, FR: {}, SA: {} };
  for (let u = 1; u <= _; u++) {
    let f = y[S], m = Object.keys(g[`${f}`]).length + 1;
    g[`${f}`][`${m}`] = u, S++, S == 7 && (S = 0);
  }
  let D = (function() {
    if (!M.day) return 0;
    if (M.day[0] == "-") {
      let u = M.day.substr(2, 2), f = Object.keys(g[`${u}`]).length + 1 - parseInt(M.day[1]);
      return g[`${u}`][`${f}`];
    } else {
      let u = M.day.substr(1, 2);
      return g[`${u}`][M.day[0]];
    }
  })();
  if (c > D || c == D && A >= (M.hour || 0)) return M.offset || "";
  let N = (function() {
    return B == "1" ? "2" : "1";
  })();
  return o[`${N}`].offset || "";
...
function $(T = false) {
  let Y = [], s = ((n) => {
    let a = {};
    for (let [e, t] of Object.entries(n)) if (typeof t == "object" && !Array.isArray(t)) {
      a[`${e}`] = {};
      let c = t;
      for (let [A, b] of Object.entries(c)) if (typeof b == "object" && !Array.isArray(b)) {
        ...
        for (let [l, d] of Object.entries(o)) {
          let E = d, B = E[0].replace(new RegExp(`^${R(e)}/${R(A)}/`), `${h.toplevel.indexOf(e)}/${A}/`);
          a[`${e}`][`${A}`][`${l}`] = [B, E[1]];
        }
      } else {
        let o = b, l = o[0].replace(new RegExp(`^${R(e)}/`), `${h.toplevel.indexOf(e)}/`);
        a[`${e}`][`${A}`] = [l, o[1]];
      }
    } else {
      let c = t, A = c[0].replace(new RegExp(`^${R(e)}/`), `${h.toplevel.indexOf(e)}/`);
      a[`${e}`] = [A, c[1]];
    }
    return a;
  })(h.db), i = (n, a = "") => {
    for (let [e, t] of Object.entries(n)) typeof t == "object" && !Array.isArray(t) ? i(t, a + e + "/") : Y.push(a + e);
  };
  return i(s), T ? JSON.stringify(Y) : Y;
}

// node_modules/add-to-calendar-button/dist/module/index.js
var atcbVersion = "2.14.0";
var atcbCssTemplate = {
...
function atcb_decorate_data_recurring_events(data) {
  ...
  function formatInTz(dateObj, timeZone, includeTime) {
    ...
    try {
      ...
      const get = (t) => parts.find((p2) => p2.type === t)?.value || "";
...
function atcb_generate_timestring(dates, language = "en", subEvent = "all", decorate = false, browserTimeOverride = false, enforceYear = false, hideTimeZone = false) {
  if (decorate) {
    dates = atcb_decorate_data_dates({ dates }).dates;
  }
...
function addLocalDays(dateObj, days, timeZone, hhmm, dateParts = null) {
  const p2 = dateParts || getPartsForTimeZone(dateObj, timeZone);
  const month0 = Number.isFinite(p2.month0) ? p2.month0 : Number.isFinite(p2.month) ? p2.month - 1 : 0;
  const baseUtc = Date.UTC(p2.year, month0, p2.day) + days * 864e5;
