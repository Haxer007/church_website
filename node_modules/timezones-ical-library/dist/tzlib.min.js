 ...
 * License: Apache-2.0
 *
 */var h=z;function $(i,e=!1){let t=D(i);if(typeof t=="string"||!t[1]||t[1]==="")return"";let T=t,s=(function(){return T[0]==""?i:T[0]})(),n="TZID="+s,a=[`BEGIN:VTIMEZONE\r
TZID:`+s+`\r
X-LIC-LOCATION:`+s+`\r
LAST-MODIFIED:`+T[1].replace(/[^\w\-:,;=+/<>]/g,"").replace(/<br>/g,`\r
`)+"END:VTIMEZONE",n];return e?JSON.stringify(a):a}function w(i,e,t){let T=D(i);if(typeof T=="string"||T[1]==null||T[1]=="")return"";let s=T;if(!e.match(/^\d{4}-\d{2}-\d{2}$/))return console.error("offset calculation failed: date misspelled [-> YYYY-MM-DD]"),"";if(!t.match(/^\d{2}:\d{2}$/))return console.error("offset calculation failed: time misspelled [-> hh:mm]"),"";if(!s[1].match(/BEGIN:DAYLIGHT/i)){let Y=s[1].match(/TZOFFSETTO:([+|-]\d{4})/i);return Y?Y[1]:""}let n=e+"T"+t+":00",a=new Date(n),r=a.getFullYear(),u=a.getMonth()+1,c=a.getDate(),A=a.getHours(),b=s[1].replace(/[^\w\-:,;=+/<>]/g,"").split("<br>"),o={1:{},2:{}},l=0;for(let Y=0;Y<b.length;Y++){let f=b[Y];if(f.startsWith("TZOFFSETTO")&&(l++,o[`${l}`]&&(o[`${l}`].offset=f.split(":")[1])),f.startsWith("DTSTART")&&o[`${l}`]&&(o[`${l}`].hour=parseInt(f.substr(17,2))),f.startsWith("RRULE")){let m=f.split(";"),L=m[1].split("=")[1],H=parseInt(L);o[`${l}`]&&(o[`${l}`].month=H,o[`${l}`].day=m[2].split("=")[1])}}o[1].month!==void 0&&o[2].month!==void 0&&o[1].month>o[2].month&&([o[1],o[2]]=[o[2],o[1]]);let d=o[1],E=o[2];if(d.month===void 0||E.month===void 0||d.offset===void 0||E.offset===void 0)return"";if(u!=d.month&&u!=E.month)return u<d.month||u>E.month?E.offset:d.offset;let B=(function(){return Object.keys(o).find(Y=>o[`${Y}`].month==u)})();if(!B)return E.offset;let M=o[`${B}`],_=["SU","MO","TU","WE","TH","FR","SA"],N=new Date(r,u,0).getDate(),S=new Date(r,u-1,1).getDay(),g={SU:{},MO:{},TU:{},WE:{},TH:{},FR:{},SA:{}};for(let Y=1;Y<=N;Y++){let f=_[S],m=Object.keys(g[`${f}`]).length+1;g[`${f}`][`${m}`]=Y,S++,S==7&&(S=0)}let Z=(function(){if(!M.day)return 0;if(M.day[0]=="-"){let Y=M.day.substr(2,2),f=Object.keys(g[`${Y}`]).length+1-parseInt(M.day[1]);return g[`${Y}`][`${f}`]}else{let Y=M.day.substr(1,2);return g[`${Y}`][M.day[0]]}})();if(c>Z||c==Z&&A>=(M.hour||0))return M.offset||"";let k=(function(){return B=="1"?"2":"1"})();return o[`${k}`].offset||""}function K(i=!1){let e=[],T=(n=>{let a={};for(let[r,u]of Object.entries(n))if(typeof u=="object"&&!Array.isArray(u)){a[`${r}`]={};let c=u;for(let[A,b]of Object.entries(c))if(typeof b=="object"&&!Array.isArray(b)){a[`${r}`][`${A}`]={};let o=b;for(let[l,d]of Object.entries(o)){let E=d,B=E[0].replace(new RegExp(`^${R(r)}/${R(A)}/`),`${h.toplevel.indexOf(r)}/${A}/`);a[`${r}`][`${A}`][`${l}`]=[B,E[1]]}}else{let o=b,l=o[0].replace(new RegExp(`^${R(r)}/`),`${h.toplevel.indexOf(r)}/`);a[`${r}`][`${A}`]=[l,o[1]]}}else{let c=u,A=c[0].replace(new RegExp(`^${R(r)}/`),`${h.toplevel.indexOf(r)}/`);a[`${r}`]=[A,c[1]]}return a})(h.db),s=(n,a="")=>{for(let[r,u]of Object.entries(n))typeof u=="object"&&!Array.isArray(u)?s(u,a+r+"/"):e.push(a+r)};return s(T),i?JSON.stringify(e):e}return G(x);})();
