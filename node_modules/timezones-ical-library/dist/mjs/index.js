 ...
 * License: Apache-2.0
 *
 */var h=z;function j(T,Y=!1){let r=p(T);if(typeof r=="string"||!r[1]||r[1]==="")return"";let s=r,i=(function(){return s[0]==""?T:s[0]})(),n="TZID="+i,a=[`BEGIN:VTIMEZONE\r
TZID:`+i+`\r
X-LIC-LOCATION:`+i+`\r
LAST-MODIFIED:`+s[1].replace(/[^\w\-:,;=+/<>]/g,"").replace(/<br>/g,`\r
`)+"END:VTIMEZONE",n];return Y?JSON.stringify(a):a}function I(T,Y,r){let s=p(T);if(typeof s=="string"||s[1]==null||s[1]=="")return"";let i=s;if(!Y.match(/^\d{4}-\d{2}-\d{2}$/))return console.error("offset calculation failed: date misspelled [-> YYYY-MM-DD]"),"";if(!r.match(/^\d{2}:\d{2}$/))return console.error("offset calculation failed: time misspelled [-> hh:mm]"),"";if(!i[1].match(/BEGIN:DAYLIGHT/i)){let u=i[1].match(/TZOFFSETTO:([+|-]\d{4})/i);return u?u[1]:""}let n=Y+"T"+r+":00",a=new Date(n),e=a.getFullYear(),t=a.getMonth()+1,c=a.getDate(),A=a.getHours(),b=i[1].replace(/[^\w\-:,;=+/<>]/g,"").split("<br>"),o={1:{},2:{}},l=0;for(let u=0;u<b.length;u++){let f=b[u];if(f.startsWith("TZOFFSETTO")&&(l++,o[`${l}`]&&(o[`${l}`].offset=f.split(":")[1])),f.startsWith("DTSTART")&&o[`${l}`]&&(o[`${l}`].hour=parseInt(f.substr(17,2))),f.startsWith("RRULE")){let m=f.split(";"),k=m[1].split("=")[1],L=parseInt(k);o[`${l}`]&&(o[`${l}`].month=L,o[`${l}`].day=m[2].split("=")[1])}}o[1].month!==void 0&&o[2].month!==void 0&&o[1].month>o[2].month&&([o[1],o[2]]=[o[2],o[1]]);let d=o[1],E=o[2];if(d.month===void 0||E.month===void 0||d.offset===void 0||E.offset===void 0)return"";if(t!=d.month&&t!=E.month)return t<d.month||t>E.month?E.offset:d.offset;let B=(function(){return Object.keys(o).find(u=>o[`${u}`].month==t)})();if(!B)return E.offset;let M=o[`${B}`],y=["SU","MO","TU","WE","TH","FR","SA"],_=new Date(e,t,0).getDate(),S=new Date(e,t-1,1).getDay(),g={SU:{},MO:{},TU:{},WE:{},TH:{},FR:{},SA:{}};for(let u=1;u<=_;u++){let f=y[S],m=Object.keys(g[`${f}`]).length+1;g[`${f}`][`${m}`]=u,S++,S==7&&(S=0)}let D=(function(){if(!M.day)return 0;if(M.day[0]=="-"){let u=M.day.substr(2,2),f=Object.keys(g[`${u}`]).length+1-parseInt(M.day[1]);return g[`${u}`][`${f}`]}else{let u=M.day.substr(1,2);return g[`${u}`][M.day[0]]}})();if(c>D||c==D&&A>=(M.hour||0))return M.offset||"";let N=(function(){return B=="1"?"2":"1"})();return o[`${N}`].offset||""}function $(T=!1){let Y=[],s=(n=>{let a={};for(let[e,t]of Object.entries(n))if(typeof t=="object"&&!Array.isArray(t)){a[`${e}`]={};let c=t;for(let[A,b]of Object.entries(c))if(typeof b=="object"&&!Array.isArray(b)){a[`${e}`][`${A}`]={};let o=b;for(let[l,d]of Object.entries(o)){let E=d,B=E[0].replace(new RegExp(`^${R(e)}/${R(A)}/`),`${h.toplevel.indexOf(e)}/${A}/`);a[`${e}`][`${A}`][`${l}`]=[B,E[1]]}}else{let o=b,l=o[0].replace(new RegExp(`^${R(e)}/`),`${h.toplevel.indexOf(e)}/`);a[`${e}`][`${A}`]=[l,o[1]]}}else{let c=t,A=c[0].replace(new RegExp(`^${R(e)}/`),`${h.toplevel.indexOf(e)}/`);a[`${e}`]=[A,c[1]]}return a})(h.db),i=(n,a="")=>{for(let[e,t]of Object.entries(n))typeof t=="object"&&!Array.isArray(t)?i(t,a+e+"/"):Y.push(a+e)};return i(s),T?JSON.stringify(Y):Y}export{j as tzlib_get_ical_block,I as tzlib_get_offset,$ as tzlib_get_timezones};
